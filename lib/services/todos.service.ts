...
import { mapTodoRow, type Todo, type TodoCategory } from '@/lib/models/todo.model';
...
export async function listTodosForCurrentUser(category?: TodoCategory): Promise<Todo[]> {
  const userId = await requireUserId();
  const rows = await todosQueries.listTodosByUser(userId, category);
