...
export async function persistGmailTokensForCurrentUser(tokens: GmailTokensResponse): Promise<void> {
  const user = await getCurrentUser();
  if (!user) {
...
async function refreshGmailAccessTokenIfNeeded(): Promise<string> {
  ...
  if (!response.ok) {
    throw new IntegrationError('Failed to refresh Gmail token', 'GMAIL_REFRESH_FAILED');
  }
...
export async function syncGmailTodos(input?: { since?: string }): Promise<GmailSyncResponse> {
