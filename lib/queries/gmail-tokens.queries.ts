...
export async function findGmailTokenForUser(userId: string): Promise<GmailTokenRow | undefined> {
  const rows = await db.select().from(gmailTokens).where(eq(gmailTokens.userId, userId)).limit(1);
  return rows[0];
}

export async function upsertGmailToken(row: typeof gmailTokens.$inferInsert): Promise<void> {
