...
import type { TodoCategory, TodoRow } from '@/lib/models/todo.model';

export async function listTodosByUser(userId: string, category?: TodoCategory): Promise<TodoRow[]> {
  if (category) {
    return db
      .select()
      .from(todos)
      .where(and(eq(todos.userId, userId), eq(todos.category, category), isNull(todos.completedAt)));
  }
